...
lab に保存・表示する。検証済みの企業ファクトは、作者本人が管理する非公開（protected）X アカウントへ
投稿する（→ [配信](./distribution.md)）。第三者への配信・外部通知は行わない。
...
| 利用範囲 | 作者本人だけが利用する。X 投稿は作者管理の非公開（protected）アカウントに限り、承認済みフォロワーを持たない運用とする。第三者再配信、外部通知、公開 Web サイトは持たない。 |
| 投稿・表示単位 | 一企業ファクトを一投稿・ローカル表示・CLI 出力の単位とする。種別ごとの抽出項目・テンプレート・抑止条件は[コンテンツ仕様](./content/common.md)に従う。 |
...
bot（TypeScript・常駐・private 利用）
  ...
  └─ 作者管理の protected X アカウントへの投稿（live モードのみ）
...
情報源・実行スケジューラ
  ...
  収集・保存 ──> 正規化 ──> ContentSpec による企業ファクト抽出
      ...
      └────────────── SQLite（正本・監査）──────────┤
                              ...
                              ┌─────────────────────┤
                              v                     v
              X（protected・live のみ）   localhost の lab / CLI
~~~

定期取得とイベント起点の取得、再取得・訂正は、収集後の正規化・企業ファクト抽出・投稿に同じ経路を使う。
情報源固有の形式は sources のアダプタ内に、X API 固有の形式は配信アダプタ内に閉じ込める。
...
| local-review | 一企業ファクト一候補の生成、重複抑止、頻度・鮮度の判定、作者本人が確認するための状態記録 |
| distribution | X API アダプタ。投稿前検証（アカウント ID・protected・フォロワー数）、dry_run / live の送信、結果照合（→ [配信](./distribution.md)） |
| operations | キルスイッチ、監視、停止・再開の監査 |

モジュール間は型付きの契約だけで接続し、分析や投稿が情報源・DB の実装詳細へ直接依存しないようにする。
...
常駐ループ（平日 07:30〜19:00 JST、10 分間隔。時間・間隔は設定 → 運用と設定）
  ...
  4. company_fact → 投稿候補と抑止記録を作成（重複・頻度・鮮度・文字数の判定）
  5. live: 投稿前検証を通過した場合だけ選抜済み候補を protected アカウントへ送信し、
     post_attempt に記録。dry_run: 送信せず post_attempt に記録のみ
  6. サイクル成功時刻を記録（watchdog 用）。要約ログを出力

作者本人は localhost の lab・CLI・自分の X タイムラインで候補と根拠を確認する。
...
| X API | Node 標準 fetch + 自前の OAuth 1.0a 署名（crypto） | 用途が投稿・自アカウント確認・結果照合に限られ、SDK 依存を増やさない |
| ログ | pino | JSON 追記ログ |
| テスト | vitest | |
...
takaneko/
  src/
    ...
    local-review/         # 投稿候補・重複抑止・頻度判定・状態記録
    distribution/x.ts     # X API アダプタ（投稿前検証・送信・結果照合・dry_run）
    storage/              # DDL 適用・型付きクエリ
    operations/           # キルスイッチ CLI・watchdog・日次要約・system 訂正 CLI
...
interface Poster {
  verifyAccount(expected: AccountId): Promise<AccountCheck>;  // protected・フォロワー数・ID 一致
  post(candidate: PostCandidate): Promise<PostResult>;        // dry_run では呼ばれない
  reconcile(attempt: PostAttempt): Promise<PostResult>;       // unknown の結果照合
}
~~~

## 安全と運用の境界

- ContentSpec の根拠・必須属性・数値検証を満たさない候補は、投稿にもローカル表示にも出さない。
- live の投稿はサイクル冒頭の投稿前検証（アカウント ID・protected・フォロワー数）を通過した場合だけ行い、
  検証失敗・確認不能はフェイルクローズとする（→ [配信](./distribution.md)）。
- 全体・情報源・ContentSpec 単位で新規処理を停止できる。
- API 認証情報、SQLite DB、収集データ、フィクスチャ原本は GitHub に保存しない。GitHub にはコードと非機密の設定テンプレートだけを保存する。
- 投稿・表示は一次情報の事実、定義済みの計算結果、または会社が明示した説明だけを扱う。個別の売買推奨や利益保証を行わない。
