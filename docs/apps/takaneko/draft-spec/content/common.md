...
このフォルダは、作者本人が X の非公開タイムラインと localhost で確認する具体的な価値と、
それを一次情報から抽出し投稿・表示する方法の正本である。
...
ならない。投稿本文とローカル表示は company_fact の確定済み項目だけから生成する。
...
ContentSpec の変更は設定版として保存し、過去の company_fact やローカル表示を再解釈しない
...
PDF を画像として読ませた OCR の数値で自動表示してはならない。文字・表構造を安定して取得できない
情報源は、原本を保存してもそのコンテンツ種別のローカル表示には採用しない。
...
行為が ContentSpec の許可語であること、必須項目・型がそろうことも確認する。検証後の投稿・表示は
固定テンプレートから生成し、LLM が返した自由文・分類根拠・因果推測は使わない。
検証に失敗した候補は保存しても投稿・表示しない。
...
4. 合格した候補だけを company_fact として保存し、投稿候補の生成へ渡す。抑止した候補は理由コードを
   analysis_suppression に保存する（→ [データモデル](../data-model.md)）。
...
- 訂正関係、対象銘柄、根拠確認の条件を満たさない。

## 投稿・表示の共通規則

- 投稿本文とローカル表示は company_fact の種別ごとの固定テンプレートだけで生成する。LLM・任意の文章生成器による
  再要約・短縮・補足を行わない。
- 企業名、証券コード、分類、主題、対象時点または会計期間を投稿本文の必須項目とする。根拠 URL は投稿に
  表示せず、内部の根拠箇所（locator）で検証可能性を担保する。lab のローカル表示では根拠箇所を併記する。
- 投稿本文は重み付き長 280 以内とする。重み付き長は X の投稿文字数の数え方に従い、
  全角系文字（CJK・かな等）を 2、半角系文字を 1、URL を一律 23 として数える（日本語主体の本文では
  おおむね 140 文字に相当する）。超える候補は抑止する。意味を保った自動要約・短縮は行わない。
- 一文書から優先投稿する company_fact は最大 3 件とする。priority、元文書内の出現順、根拠箇所 ID の順に
  選ぶ。priority は文書内の選抜だけに使い、企業価値や株価への影響を評価する
  値ではない。

## 重複キーの原則

投稿候補の重複キーは、company_id、ContentKind、種別ページが定める dedupeFields の意味属性だけから作る。
文書の ID・内容ハッシュ、根拠箇所の内容ハッシュ、ContentSpec 版、配信ルール版は含めない。
設定の版上げ、再正規化、同一事実を載せた別文書から、同じ内容を重複投稿しないためである。

訂正を表す company_fact（correction_kind = source または system）は、訂正元と同じ重複キーでも
投稿でき、訂正表示を含める。dedupeFields の属性が欠けるときは、ContentSpec が明示的に代替属性を定めない限り投稿しない。
