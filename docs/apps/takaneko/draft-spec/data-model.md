...
| analysis_suppression | 抽出・検証段階の抑止記録（company_fact を作らなかった候補の理由） | 実行 ID、文書 ID、種別、metric、理由コード、詳細 |
| company_fact | 検証済みの一主題の企業理解用事実 | 銘柄 ID、種別、主題、対象時点・会計期間、属性 JSON、根拠箇所、財務値参照、ContentSpec 版、品質状態、訂正種別 |
| post_candidate | 一企業ファクトから作成した投稿候補（dry_run・live 共通） | 本文、テンプレート ID、重複キー、判定結果、抑止理由、対象集合・配信ルール版 |
| post_attempt | 投稿試行の監査記録（dry_run を含む） | 候補 ID、試行時刻、状態、X 投稿 ID、失敗分類 |
...
analysis_run、analysis_suppression、company_fact、post_candidate、post_attempt、operation_event は
監査対象の正本として無期限に保存する。
...
- 抽出・検証段階で company_fact を作らなかった候補は、analysis_suppression に理由コードを残す。
...
-- 同一 code の有効期間を重複させない（重複しないことは設定取り込み時に検証する）。
-- 有効な行は code ごとに高々 1 行とし、開示一覧のコード表記との照合規則は収集側で定める。
CREATE UNIQUE INDEX idx_instrument_active_code
  ON instrument (code) WHERE valid_to IS NULL;
...
CREATE TABLE source_record (
  ...
  published_at TEXT,            -- 情報源が公開時刻を提供しない場合は NULL（分析には使わない）
...
CREATE TABLE financial_fact (
  ...
  role TEXT NOT NULL,           -- actual / prior_actual / forecast / previous_forecast / revised_forecast / dividend_actual / dividend_forecast
...
CREATE TABLE analysis_run (
  ...
  finished_at TEXT,
  status TEXT NOT NULL          -- ok / suppressed / error（partial は suppression 行で表す）
) STRICT;

-- 抽出・検証段階で company_fact を作らなかった候補の理由。
-- 一文書から一部 metric だけ確定した場合も、確定しなかった metric ごとに 1 行残す。
-- lab の抑止統計（理由コード別・種別別の件数推移）はこのテーブルと post_candidate から作る。
CREATE TABLE analysis_suppression (
  id INTEGER PRIMARY KEY,
  analysis_run_id INTEGER NOT NULL REFERENCES analysis_run(id),
  document_id INTEGER NOT NULL REFERENCES disclosure_document(id),
  kind TEXT NOT NULL,           -- ContentKind
  metric TEXT,                  -- metric 単位の抑止のみ。文書単位の抑止は NULL
  reason_code TEXT NOT NULL,    -- 各種別ページの抑止理由コード
  detail TEXT,
  created_at TEXT NOT NULL
) STRICT;
...
CREATE TABLE company_fact (
  ...
  corrects_x_post_id TEXT,      -- system 訂正が対象とする X 投稿 ID（→ 配信）
  document_id INTEGER NOT NULL REFERENCES disclosure_document(id),
  analysis_run_id INTEGER NOT NULL REFERENCES analysis_run(id),
  content_spec_version_id INTEGER NOT NULL REFERENCES config_version(id),
  created_at TEXT NOT NULL
