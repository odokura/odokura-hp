...
VIP・ExtraVIPへの招待、XXGGLLの取得、役務相談の開始に本人確認を要求しない。法令上の本人確認が必要な
対象コンテンツ・役務は提供しない。
...
個別役務の情報共有同意を管理する。クリエイターは四半期ごとの受入上限だけを設定し、受け取る情報項目は
役務種別ごとの審査済み契約テンプレートが固定する。クリエイターが任意の属性カテゴリや自由記述項目を追加してはならない。
...
- プログラム終了後は新規支援を停止し、証票を終了済み関係記録として保持する。現在保有者への発行者向け開示は、[属性開示・決済事業者確認](../foundation/identity-disclosure.md)に従う。
