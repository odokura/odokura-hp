...
flowchart TB
    ...
    creatorConsent --> legal{開示・利用に法的KYCが必要か}
    legal -->|不要| payment[別決済]
    legal -->|必要| kyc[対象に必要な範囲だけKYC]
    kyc --> result{確認結果}
    result -->|利用可| payment
    result -->|利用不可| close
...
- 禁止事項、安全中止条件、通報方法
- 成果物の利用権、公開範囲、保存期間
- 対象コンテンツ・役務に法的KYCが必要か。必要な場合は確認項目、目的、保存期間

クリエイターは匿名打診、条件案、最終条件の各段階で見送れる。最終応諾前は役務義務を負わず、
見送り理由の詳細をユーザーへ開示する義務もない。

VIP・ExtraVIPであること自体、XXGGLLの購入、役務相談の開始にはKYCを要求しない。KYCは、
契約対象のコンテンツ・役務を開示・利用するため法的に必要な場合だけ、利用直前に運営が実施する。

## 責任分担

| 主体 | 責任 |
| --- | --- |
| 運営 | 商品・役務の分離表示、条件テンプレート、同意、決済、上限管理、記録、通報、必要時の条件付きKYC |
