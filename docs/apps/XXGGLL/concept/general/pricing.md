...
本ページは、新規発行価格と、クリエイターが設定した上限以下でAIが実発行数を決める
...
- 無料XXGGLLは無料・譲渡不可とし、発行数AI算出を行わない。
...
クリエイターはランクごとに次のいずれかを選ぶ。既定はAI自動算出とする。

| 価格モード | ルール |
| --- | --- |
| AI自動算出 | 確認済み需要と成立実績からAIが価格を決める。クリエイター・運営の承認を挟まない |
| クリエイター任意価格 | 決済可能な最低金額以上で設定できる。AIは市場差と予測消化率を警告するが、変更を強制しない |
...
初回基準は固定価格ではない。AIは次を使い、発売後30日以内の予測消化率80%以上を守る範囲で、
クリエイターの予測収益を最大化する価格を選ぶ。
...
任意固定コンテンツの有無、制作費、将来の投稿、返信、物品、イベント、個別役務はAI価格の
加算要素にしない。
...
警告し、二段階確認後に公開する。基準価格を超える場合も、AI価格との差と予測消化率を警告する。

### VIP・ExtraVIP

- VIPは同じクリエイターの最上級価格、本人の過去支援帯、非公開需要を使ってAI算出する。
...
| --- | --- |
| クリエイター入力 | 発行上限のみ |
| AI入力 | 重複・不正補正済み待機数、価格感応度、過去の購入率、キャンセル・返金・不正率、二次流通流動性 |
| AI目標 | 30日以内の予測消化率80%以上を維持しながら予測収益を最大化 |
| 自動決定 | AI算出数を最終値とし、クリエイター・運営は上書きしない |
...
VIPはクリエイターが四半期ごとの受入上限を設定し、AIが招待済み需要に基づきその範囲内で発行する。
