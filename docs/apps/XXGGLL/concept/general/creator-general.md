...
本ページは、無料・認定・上級認定クリエイターが一般向けXXGGLLプログラムを作成・運用する際の
...
- 運営が決済、台帳、証票生成、属性同意、通知、通報、返金と、必要時の条件付きKYCを処理する。
...
```mermaid
flowchart TB
    register[提供者アカウント登録] --> draft[無料・一般プログラムを下書き]
    draft --> verify[本人確認・正規提供者確認・精算先登録]
    verify --> setup[発行上限・価格モード・属性受入設定]
    setup --> optional[任意固定コンテンツを登録するか選ぶ]
    optional --> review[運営の自動検査・必要時審査]
    review --> demand[KYC不要の待機リストで需要計測]
    demand --> issue[AIが発行数を確定]
    issue --> publish[販売開始]
    publish --> operate[運営が取得・継続・譲渡を自動処理]
```

無料クリエイターは無料・一般プログラムを作成できるが、本人確認、正規提供者確認、精算先登録を
完了するまでは公開・発行できない。認定後は一般・上級、上級認定後は最上級まで発行できる。
...
| 発行上限 | 同時に有効にできる上限 |
| 価格モード | AI自動価格またはクリエイター任意価格 |
| 属性受入 | 受け取りたい任意属性項目、自由記述を受け取るか |
| 任意固定コンテンツ | 販売前に完成済みの素材を登録するか。登録しなくてもよい |
| 継続 | プログラム全体を継続するか終了するか |

実発行数はAIが上限以下で決める。クリエイターと運営はAI確定数を上書きしない。
...
これらが必要な商品・サービスは、XXGGLLから分離したS1別商品またはS2 VIP・ExtraVIP役務として扱う
（→ [任意提供・個別役務](../benefits/templates.md)）。
...
支援者一覧、属性、メッセージの閲覧は任意であり、未閲覧を違反や評価低下として
扱わない。

## 属性とメッセージ

- クリエイターは受け取る属性カテゴリを事前設定でき、不要なカテゴリを無効化できる。
- 個別通知は行わない。クリエイターがダッシュボードを開いたときに集計を確認できる状態を既定とする。
- 個別プロフィールを閲覧しても、ユーザーへ閲覧状況を通知しない。
- CSV等の一括エクスポートは既定で許可しない。
- 迷惑な属性・メッセージは1操作で非表示・通報でき、以後の調査は運営が行う。
- 属性を購入者の承認・拒否、差別的選別、外部営業、私的連絡へ使わない。

詳細は[属性開示・条件付き本人確認](../foundation/identity-disclosure.md)を正本とする。
...
受け取らない。海外クリエイターは、登録する居住・事業国がStripe Connectの対象で、運営プラットフォームから
その国への送金経路が有効な場合に限り出金できる。対象外またはStripe審査未完了の場合、出金は保留し、新規の
別送金手段は用意しない。

支援者データは、人数、ランク、支援額帯、継続期間、関心タグを既定の集計とする。正確な個人支出額は
業務上必要な権限を持つ提供者だけが確認し、営業リストとして持ち出せない。

## 継続・終了・安全

- 一時休止機能は設けず、販売継続またはプログラム終了を選ぶ。
- クリエイターが明示終了した場合、運営が次回以降の継続支援の自動更新、新規発行、属性共有を停止し、
  保有者へ通知する。支払済み期間は満了させ、証票・来歴・公開済みの表示はXXGGLL内に保持する。
