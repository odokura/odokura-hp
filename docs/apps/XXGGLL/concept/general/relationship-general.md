...
本ページは、一般ユーザーが無料・一般・上級・最上級XXGGLLを取得し、属性を共有し、継続支援、
譲渡、退出するまでの体験を扱う**一般ユーザー関係性フローの正本**である。

一般ルートではクリエイターの購入承認を必要としない。XXGGLL取得だけで、クリエイターによる
制作、配信、返信、閲覧等の役務は発生しない。属性同意と条件付きKYCは
[属性開示・条件付き本人確認](../foundation/identity-disclosure.md)、証票状態は
...
```mermaid
flowchart TB
    discover[XXGGLLを知る・閲覧する] --> select[クリエイターとランクを選ぶ]
    select --> inventory{取得可能か}
    inventory -->|在庫なし| wait[通知先だけ登録して待機]
    wait --> notice[入荷通知]
    notice --> inventory
    inventory -->|在庫あり| account[最小限のアカウント作成・ログイン]
    account --> route{取得方法}
    route -->|無料| acquireFree[無料証票を取得]
    route -->|新規発行| primary[一次支援を決済]
    route -->|二次流通| secondary[証票価格と継続支援を決済]
    acquireFree --> active[関係証票を保有]
    primary --> active
    secondary --> active
    active --> profile[共有する属性・応援表現を選ぶ]
    profile --> continue[保有・継続支援・支援履歴を確認]
```

- 閲覧にはアカウントを求めない。待機リストは通知先だけで登録できる。
- 無料取得と購入には証票を保持する最小限のアカウントだけを求め、KYCを行わない。
- 一次購入、二次購入、継続支援、出品、通常の属性共有にもKYCを要求しない。
...
| --- | --- | --- | --- |
| 閲覧 | クリエイター、ランク、価格、発行上限、役務なしの説明を確認する | なし | 閲覧だけでは関係を作らない |
| アカウント作成 | メールアドレス等の最小限の情報で証票の保有先を作る | なし | 一般ユーザーになる。KYCは行わない |
| 取得 | 無料取得、一次支援、二次取得のいずれかを選ぶ | 個別承認しない | 証票と現在の保有関係を記録する |
| 属性共有 | 表示名、年代、地域、興味、応援理由等から共有項目を選ぶ | ダッシュボードで集計を任意に見る | 共有先・項目・目的・同意を記録する |
...
法的に本人・年齢・資格確認が必要なコンテンツまたは役務を利用する場合だけ、利用直前に対象範囲の
KYCを行う。KYCを完了できなくても、証票の購入・保有・譲渡には影響させない。
...
- 運営は現在支援中の表示、継続記念、支援履歴、属性共有、クリエイター向け集計を維持する。
...
プログラム終了は保有者に落ち度がないため、運営は証票を一方的に回収しない。プログラム終了は次回以降の
自動更新を止めるものであり、支払済み期間は満了まで維持する。満了後も、証票と来歴およびXXGGLL内で公開済みの
...
終了後の表示・記録保持は、クリエイター利用規約への同意を前提にする。自動更新・終了の最終画面と
利用規約は、公開前に[画面・コンソール構成](../../app-spec/screens.md)との一致を確認する。
