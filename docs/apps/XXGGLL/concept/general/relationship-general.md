...
flowchart TB
    ...
    primary --> active
    secondary --> active
    active --> profile[プロフィールと発行者への共通公開を設定する]
...
| アカウント作成 | メールアドレス等、アカウント名、出身国・地域の自己申告で証票の保有先を作る | なし | 一般ユーザーになる。本人確認は行わない |
| 取得 | 一次支援または二次取得を選ぶ | 個別承認しない | 証票と現在の保有関係を記録する |
| プロフィール・発行者向け公開 | アカウント名、出身国・地域、任意プロフィールを編集し、任意プロフィールの共通公開を選ぶ | 匿名集計を任意に見る。ownerは自分の発行XXGGLLの現在保有者の限定項目を確認できる | 自動開示、公開フラグ、閲覧を記録する |
...
- 運営は証票の現在状態、継続記念、支援履歴、発行者向けの自動開示・任意プロフィール公開、クリエイター向け匿名集計を維持する。
...
| --- | --- | --- | --- |
| A 継続停止 | 自動更新を停止、または決済失敗後の猶予満了 | 非継続で保持。90日以内は再開・出品・返還可能 | 過去履歴を保持。現在保有者である限り発行者向け開示は継続 |
| B 再発行 | 継続中または猶予中に二次流通の再発行を申し込む | 番号・来歴を新保有者へ引き継ぐ。前保有者へは再発行クレジットを付与 | 前保有者の履歴・属性・メッセージは移転せず、発行者への開示資格も即時に終了 |
| C 自主返還 | 対価を求めず即時退出 | 運営が回収。プログラム継続中なら新規発行枠へ戻せる | 過去履歴を保持。発行者への開示を即時に終了し、退出理由は外部表示しない |
| D プログラム終了 | クリエイターの終了宣言または運営による終了 | 次回以降の自動更新を止め、支払済み期限の満了後に終了済みとして期限なく保持・譲渡・放棄可能 | 新規支援は停止。現在保有者である限り発行者向け開示は継続し、過去履歴を保持 |
| E 強制失効 | 規約違反・安全上の判断 | 運営が回収・アーカイブ | 発行者への開示を即時に終了。必要な不正対策記録を保持し、外部への理由表示は最小限 |
...
表示は終了済みの関係記録として残す。終了後の二次取得者には、新しい支援関係が始まらないことを明示する。
取得成立により現在保有者となる場合の発行者への開示は、[属性開示・決済事業者確認](../foundation/identity-disclosure.md)に従う。
...
- ユーザーはCreatorごとに、一次支援、継続支援、証票支援、ロイヤリティ寄与、保有期間、確認済み商品購入を来歴と月別グラフで確認できる。
- グラフは本人が実際に支払った額、Creatorへの支援額、税・手数料、二次流通、再発行クレジット、返金・取消しを区別する。正規販売元が金額と通貨を確認できない商品購入は来歴だけに表示し、推定額を合算しない。
- 支援実績と保有期間はユーザーランク判定へ反映し、本人へ反映項目と現在段階を示す。VIP・ExtraVIPは自動昇格させず、運営審査、招待、本人承諾を必要とする。
- 支援者内の順位は正確な金額ではなく、「上位10%」等の帯で本人だけに表示する。
- クリエイターには現在支援者数、ランク別人数、支援額帯、継続期間、公開可能なプロフィールの匿名集計を表示する。
- 発行者ownerが現在保有者の限定情報を確認できる場合も、個人支援者を選別、承認、営業する用途には使わない。
