...
- 無料証票はPublic Betaの対象外とし、譲渡・二次流通・継続支援を行わない。
...
- 外部案内リンクの閲覧、待機、一次購入、二次購入、継続支援、出品に本人確認を要求しない。無料取得はPublic Betaの対象外とする。
...
- 7日以内に成功しなければ非継続へ移し、現在支援者表示だけを停止する。現在保有者である限り、発行者向け開示は[属性開示・決済事業者確認](../foundation/identity-disclosure.md)に従う。
...
追加商品・VIP・ExtraVIP個別サービスのクリエイター対価は、それぞれの商品・個別サービス契約に基づく対価であり、
...
Accountの本人確認、口座確認および送金可否がすべて有効になるまでは送金を実行しない。出金対象は日本のJPY対応
Connected Accountに限定する。対象外またはStripeの審査未完了に対して、運営は代替の銀行送金を行わない。
...
| 追加商品 | 商品契約に定める対価 | XXGGLL支援実績へ加算しない | 証票原価へ加算しない |
| VIP・ExtraVIP個別サービス | 個別サービス契約に定める対価 | XXGGLL支援実績へ加算しない | 証票原価へ加算しない |
...
- 同時に有効な数は自動算出で確定した発行数を超えない。
- 終了済み証票はアーカイブし、再発行しない。

## プログラム終了

プログラム終了は、以後の自動更新を終了することであり、XXGGLLの証票・来歴・公開済みの表示を削除することではない。
終了確定時刻以後は、新規発行、待機リスト、次回以降の継続決済を停止する。現在保有者への発行者向け開示は、現在の保有関係が残る限り継続する。
...
- 終了後の二次取得では継続支援を開始しない。取得成立により現在保有者となる場合の発行者向け開示は、[属性開示・決済事業者確認](../foundation/identity-disclosure.md)に従う。
- 放棄した終了済み証票はアーカイブし、再発行しない。
- 追加商品とVIP・ExtraVIP個別サービスは、証票終了とは分けて各契約条件で精算する。
