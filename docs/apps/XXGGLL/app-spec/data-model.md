...
| fixed_content_asset | 任意固定コンテンツ（R2）の登録 | 公開時に定めた範囲で引き継ぐ |
| vip_invite | VIP・ExtraVIP招待の審査・決定 | — |
| concierge_case / concierge_message | VIPコンシェルジュの専用スレッドと状態 | 引き継がない |
| s1_offering | S1別商品の商品情報・数量上限 | — |
| service_engagement | S1別商品・S2 VIP・ExtraVIP役務の個別契約（S1はs1_offering、S2はconcierge_caseと紐づく） | 引き継がない |
| moderation_report | 通報・調査対象 | — |
| audit_log_entry | 全操作の追記専用の監査ログ | — |

## 物理スキーマ

全エンティティのDDLを示す。すべて共通の規約（UUID主キー、外部キー、追記専用の履歴テーブルは更新・削除
しない）に従う。細部（文字数上限、追加インデックス等）は実装時に調整してよいが、テーブル構成・主要な
制約はここを正本とする。
...
-- ログイン・通知用の連絡先は account_contact に分離し、閲覧権限を最小化する（→ セキュリティ）。
...
CREATE TABLE creator_staff_member (
  ...
  scope TEXT NOT NULL, -- creator_profileが許可した閲覧範囲。MVPは属性集計閲覧のみを想定
...
CREATE TABLE issuance_program (
  ...
  price_mode TEXT NOT NULL DEFAULT 'ai_auto' CHECK (price_mode IN ('ai_auto','creator_set')),
...
CREATE TABLE concierge_case (
  ...
  case_type TEXT NOT NULL CHECK (case_type IN ('attribute_share','service_request')), -- 通常の属性・希望共有か、S2役務相談か
...
-- 件数で数える。quantity_limitに達したら運営がstatus='sold_out'にする（→ 構想の任意提供・個別役務のS1）。

CREATE TABLE service_engagement (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  kind TEXT NOT NULL CHECK (kind IN ('s1','s2')),
  user_account_id UUID NOT NULL REFERENCES account(id),
  creator_id UUID NOT NULL REFERENCES creator_profile(id),
  s1_offering_id UUID REFERENCES s1_offering(id), -- kind='s1'のときだけ設定する
  ...
  price NUMERIC(14,2),
  currency TEXT NOT NULL DEFAULT 'USD', -- 既定はUSD表記（→ 決定ログ）。実際に対応可能かはStripeの仕様次第
  contracted_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CHECK (
    (kind = 's1' AND s1_offering_id IS NOT NULL AND concierge_case_id IS NULL) OR
    (kind = 's2' AND s1_offering_id IS NULL)
  )
);
-- 証票のライフサイクル（certificate.state）とは独立したステートマシン。証票が非継続・終了・回収されても、
-- 成立済みのservice_engagementはtermsの条件に従って履行・代替・返金する（→ 構想の任意提供・個別役務）。

CREATE TABLE ledger_entry (
  id BIGSERIAL PRIMARY KEY,
  transaction_id UUID NOT NULL, -- 同一決済に属する当事者別の行を束ねるID
  kind TEXT NOT NULL CHECK (kind IN ('primary_issue','continued_support','certificate_boost','secondary_reissue','s1_purchase','s2_service','refund')),
  certificate_id UUID REFERENCES certificate(id), -- S1/S2で証票に紐付かない場合はNULL
  service_engagement_id UUID REFERENCES service_engagement(id), -- S1/S2の場合、対象契約への参照
  ...
  currency TEXT NOT NULL DEFAULT 'USD', -- 取引・分配金の計上通貨。実際の払出通貨とは分離し、換算がある場合はpayout_requestに記録する
...
-- 再発行・継続支援だけで、S1/S2・VIP・物販その他クリエイター個別の役務には使えない。
...
CREATE TABLE payout_request (
  ...
  settlement_currency TEXT, -- 実際の送金通貨。Stripeの対象国・通貨の確認後にだけ設定する
  fx_rate NUMERIC(20,10), -- 計上通貨と送金通貨が異なる場合の換算レート
...
-- creator_profile.stripe_connected_account_idに保存し、Stripeから送金可と確認できた時点で
-- stripe_payouts_enabled_atを設定する。海外クリエイターは、payout_country、Connected Accountのサービス契約、
-- 運営プラットフォームからの送金経路および送金通貨がStripeで有効な場合だけ処理する。
-- 計上通貨と送金通貨が異なる場合、Stripeから得た確定額・通貨・換算レート・時点を本テーブルに保存する。
...
-- 譲渡成立時は、対象証票に紐づく前保有者の全項目をrevoked_atで一括終了する（アプリケーション層）。
...
-- 同一証票に同時に有効な再発行申込みを1件だけに制限する。在庫仮押さえの排他制御はNF-02・技術検証V-4を正本とする。
...
-- （→ 構想の任意提供・個別役務）。購入者ごとの差し替え・個別制作は行わない（1つのasset_refを全保有者へ配信）。
...
- 発行上限と実発行数の不変条件（`certificate.state IN ('active','inactive')` の件数に、有効な
  ...
  （[技術検証計画](./tech-verification.md)のV-4）。一次発行の15分仮押さえは `issuance_reservation` が担い、
...
- `stripe_webhook_event.stripe_event_id` の主キーにより、署名検証済みのStripeイベントを同じイベントIDで
  再処理しない。`account.updated`は、対象の`creator_profile`が存在する場合だけ
...
- `payout_request` の金額は、対応する `ledger_entry` の未精算合計（精算可能日の到来し、確認中の`payment_review`がない有効な報酬・別契約対価 − 返金取消額 −
  申請済み・処理中・完了済みpayout_requestの合計）を超えてはならない。この超過チェックもアプリケーション層で行う。
- `payout_request` を`processing`へ遷移させる前に、対応するcreator_profileの`stripe_connected_account_id`、
  `stripe_payouts_enabled_at`、`payout_country`およびStripeが返す送金通貨・送金可否を確認する。不成立なら
...
- `s1_offering.quantity_limit` を超える `service_engagement`（`kind='s1'`、`status NOT IN ('cancelled','refunded')`）
  を成立させない。発行上限と同様、行ロックまたはシリアライザブル分離レベルのトランザクションで保証する
  （[技術検証計画](./tech-verification.md)のV-4に準ずる）。上限到達時は `s1_offering.status` を `sold_out` にする。
