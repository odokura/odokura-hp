...
| `SC-P05` | `/pricing` | 静的な費用説明 | Fan支払、Creator報酬、精算、停止、返金を区別する | `B-01`から`B-04`のいずれかが未解決中は購入可能表示と手数料仮定を禁止する | [費用](./public-costs.md)の`PC-*` |
...
| `SC-P10` | `/entry/[publicLinkId]/confirm` | `RM-AcquisitionQuote` | 対象、価格、手数料、合計、通貨、期限、継続、停止、返金条件を示し、一度だけ確定する | 在庫切れ、予約期限切れ、決済失敗、`pending_external`、競合を分ける。`B-01`から`B-04`のいずれかが未解決中は確定不可 | quote整合性、冪等性、再課金防止、期限、`B-01`から`B-04`のgateをE2E確認する |
...
| `SC-F08` | `/fan/profile` | `RM-OwnProfile` | 閲覧モードを初期表示し、`編集する`でプロフィール値の編集モードへ切り替える。`プロフィールを保存`と`キャンセル`はプロフィール値だけを扱い、任意プロフィールの各チェックは一操作でその項目の共有設定だけを即時保存する | 保有0件でも保存可、開示先0件を示す。共有失敗は変更した項目を確定値へ戻し、プロフィール値保存失敗は入力を維持する | 新規6項目の共有既定、既存項目状態維持、項目単位のowner開示、staff・前保有者への不開示、競合・監査を確認する |
...
- `B-01`から`B-04`のいずれかが未解決中は、`ready`から`submitting`へ進めない。
