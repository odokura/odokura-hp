...
| 任意プロフィール | fieldごとのunique respondent | 共通共有が有効で、当該fieldに保存値がある人を固定カテゴリへ割当 | そのfieldの回答者。全保有者を回答者として扱わない |
...
- 任意プロフィールは共通共有が有効な現在保有者の入力済みfieldだけを集計する。
...
- 共有無効、未回答、fieldごとに回答数が異なる場合にcoverageと分母が一致する。
