...
[セキュリティ詳細](./security.md)、決済・認可・状態遷移を確定させる条件は[コントロール仕様](./controls.md)、
日常手順と担当は[運用](./operations.md)を参照する。
...
| 実装が満たす認証・データ・決済要件 | [セキュリティ詳細](./security.md) | API、Cookie、トランザクション、保持 |
| 業務操作を確定してよい条件 | [コントロール仕様](./controls.md) | 許可、拒否、失敗、証跡、テスト |
| 役割別に何を閲覧・操作できるか | [要件](./requirements.md) | 権限マトリクス、受け入れ条件 |
| Railwayで誰が何をいつ行うか | [運用](./operations.md) | 担当、頻度、手順、証跡、復旧 |
| 何をどの制約で保存するか | [データモデル](./data-model.md) | DDL、外部キー、制約、状態遷移 |
...
| カテゴリ | 設計方針 | 主な詳細正本 |
| --- | --- | --- |
| GOV: 統治 | リスク受容者、運用担当、変更証跡、例外期限を明確にする | 本書、[運用](./operations.md) |
| IAM: 認証・認可 | 通常利用者、Creator、staff、Ops、Railway管理者の境界を分離する | [セキュリティ詳細](./security.md)、[要件](./requirements.md) |
| APP: アプリ防御 | サーバー側検証、CSRF、redirect・Host制限、レート制限、安全な応答を共通化する | [セキュリティ詳細](./security.md)、[コントロール仕様](./controls.md) |
| DATA: データ保護 | 最小収集、用途別read model、同意、保持、非表示、匿名化を設計する | [セキュリティ詳細](./security.md)、[データモデル](./data-model.md) |
| PAY: 決済・台帳 | Stripe委譲、署名、冪等性、追記台帳、返金・送金前の再照合を行う | [コントロール仕様](./controls.md) |
| PLAT: Railway基盤 | private networking、環境分離、sealed variable、最小権限、config as codeを使う | 本書、[運用](./operations.md) |
| DET: 検知 | アプリ監査、Railwayログ・metrics・webhook、セキュリティ通知を組み合わせる | [運用](./operations.md) |
| RESP: 対応 | 受付、判定、封じ込め、証拠保全、連絡、再発防止を担当付きで実行する | [運用](./operations.md) |
| REC: 復旧 | rollback、PITR、世代バックアップ、整合性照合、復旧訓練を行う | 本書、[運用](./operations.md) |
| SDLC: 開発 | lockfile、CI、秘密値検査、脆弱性例外、受け入れ条件をrelease gateにする | [セキュリティ詳細](./security.md)、[運用](./operations.md) |
...
- UIの非表示は認可の代替にしない。未認証・対象なし・権限なしの詳細を外部へ区別しない画面では同じ応答を返す。
...
- 現行の運用入口はDev専用の`npm run ops:setup`とし、固定Dev targetへのSSH委譲以外を拒否する。固定Dev targetをproductionへ流用しない。
- productionの初回登録・復旧・解除は、Devと同じ操作境界を保つproduction専用固定target、対象accountの一意性、actionごとの固定理由、
...
| environment | `production`と`staging`を分離する | [Railway environments](https://docs.railway.com/environments)は環境ごとにservice・変数・networkを分離する |
| PR環境 | baseをstagingとし、本番sealed secret・本番データを複製しない | sealed variableはPR環境や複製環境へコピーされないため、専用の非本番値を設定する |
...
Enterpriseを採用しない間は、project memberを必要最小限にし、stagingを通常調査先とすることで補う。
...
四半期drillはPITRまたは外部論理backupから、通常stagingとは別の一時recovery serviceへ復元する。Railwayのnative volume backupは、復元時に元serviceの
...
- Railway healthcheckは継続監視ではない。Public OriginとOps Originに外形監視を置き、Opsは認証前に機密情報を返さない専用health応答を使う。
...
頻度、証跡、停止、復旧手順は[運用](./operations.md)を正本とする。
...
| 対象 | 現状 | 必要な対応 |
| --- | --- | --- |
| Web deploy | `railway.toml`にpreflight、migration、`/api/health`、100秒timeout、`ON_FAILURE`最大10回がある | releaseごとに実際のRailway適用値を照合する |
| Cron deploy | 専用TOMLにmigration、月次schedule、`NEVER`がある | 失敗通知と冪等な手動再実行を確認する |
| Dev Ops初期設定 | `npm run ops:setup`がRailway本人確認後に固定Dev targetへSSH委譲し、対象accountが一意でない場合は停止する | production targetへ流用せず、Devでbootstrap・recover・revokeを検証する |
| overlap / draining | repositoryのTOMLに未定義 | 30秒の設定とSIGTERM試験を実装Issueで追加する |
