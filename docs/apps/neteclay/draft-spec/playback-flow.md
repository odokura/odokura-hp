...
| fading | 消灯。固定フェードアウトで停止へ向かう。終了時の stopReason は pendingStopReason で保持する | deepening_elapsed、manual_stop、safety_timeout | fade_complete、soothe_again、emergency_stop |
...
| intro / intro_hold / deepening | manual_stop | fading へ。pendingStopReason は manual | fading へ。pendingStopReason は manual |
...
- 「寝るまでの時間」として表示する値は、最新の「もう一度あやす」以降に押された「寝た」までの時間とする。
  「もう一度あやす」がない場合だけ、再生開始から「寝た」までの時間とする。
...
- 就寝フロー中に全画面モーダルを出さない。
- 手動停止のフェード中は pendingStopReason を manual として保持し、フェード完了時に stopReason を manual にする。
- 安全停止は、手動モードで「寝た」が押されないまま再生開始または最後の「もう一度あやす」から 90 分経過したときの停止。
  親の操作ではないため stopReason は safety_timeout とし、音は通常停止と同じ短いフェードで止める。
- 緊急停止は長押しで即停止するが、親の明示操作なので stopReason は manual とする。
- 停止後はホームへ戻る。
...
- ホームの「だいたい○分で寝つく」は、手動モードでは sleepLatencyMs、十分な記録がない場合は自動モードの推定時刻を補助的に使う。
